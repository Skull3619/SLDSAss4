...
def choose_next_indices(
    ...
    # normalized feature space for diversity logic
    ...
    # uncertainty-based methods need a provisional model
    ...
    if strategy == "Uncertainty sampling":
        ...
        u = norm01(uncertainty)
        d = norm01(div_score)
        combo = hybrid_weight * u + (1.0 - hybrid_weight) * d
    ...
    if minority_protection == "Off":
        chosen_local = base_order[: min(batch_size, len(base_order))]
        return pool_idx[chosen_local]
    ...
    if minority_protection == "Minority-priority acquisition":
        ...
        chosen = minority_order + other_order
        return pool_idx[np.array(chosen[:batch_size])]
...
def run_single_sequential_experiment(
    ...
    batch_size: int,
    budget_mode: str,
    minority_handling: str,
    minority_protection: str,
    initial_seed_strategy: str,
    initial_seed_fraction: float,
    distance_metric: str,
    hybrid_weight: float,
    evaluation_metric_ui: str,
    baseline_model: str,
    random_state: int,
    ...
    # acquisition budget
    if budget_mode == "Fraction of training set":
        budget = max(5, int(round(n_train * training_subset_fraction)))
    else:
        budget = max(5, min(st.session_state.get("q2_num_samples_budget", 40), n_train))

    seed_size = max(2, int(round(n_train * initial_seed_fraction)))
    ...
    seed_idx = select_seed_indices(
        ...
        y_train,
        seed_size=seed_size,
        strategy=initial_seed_strategy,
        random_state=random_state,
    ...
    if acquisition_mode == "Single-sample acquisition":
        step = 1
    else:
        step = batch_size
    ...
    while len(labeled_idx) < budget:
        ...
        # evaluate at current acquisition size
        X_cur = X_train.iloc[labeled_idx].reset_index(drop=True)
        y_cur = y_train.iloc[labeled_idx].reset_index(drop=True)

        if oversample_mode:
            X_fit, y_fit = apply_random_oversampling(X_cur, y_cur, random_state=random_state)
        else:
            X_fit, y_fit = X_cur, y_cur

        model = build_model(baseline_model, class_weight_mode=class_weight_mode, random_state=random_state)
    ...
    if oversample_mode:
        X_fit, y_fit = apply_random_oversampling(X_final, y_final, random_state=random_state)
    else:
        X_fit, y_fit = X_final, y_final
...
with st.sidebar:
    ...
    training_subset_fraction = st.select_slider(
        "Training subset fraction",
        options=[0.20, 0.40, 0.60, 0.80, 1.00],
        value=0.40,
    )

    baseline_model = st.selectbox(
        "Baseline model",
        ["random_forest", "extra_trees", "logreg", "svm_rbf"],
        index=0,
    )

    selection_strategy = st.selectbox(
        "Selection strategy",
        ["Random", "Stratified random", "Diversity sampling", "Uncertainty sampling", "Hybrid: uncertainty + diversity"],
        index=1,
    )

    sequential_mode = st.selectbox(
        "Sequential acquisition mode",
        ["Single-sample acquisition", "Batch acquisition"],
        index=1,
    )

    if sequential_mode == "Batch acquisition":
        batch_size = st.select_slider("Batch size", options=[1, 5, 10, 20], value=10)
    else:
        batch_size = 1

    budget_mode = st.selectbox(
        "Acquisition budget mode",
        ["Fraction of training set", "Number of samples"],
        index=0,
    )

    if budget_mode == "Number of samples":
        st.session_state["q2_num_samples_budget"] = st.number_input(
            "Number of acquired training samples",
            min_value=5,
            max_value=10000,
            value=40,
            step=5,
    ...
    minority_handling = st.radio(
        "Minority-class handling",
        ["None", "Random oversampling", "Class-weighted training", "Oversampling + class-weighted training"],
        index=2,
    )

    minority_protection = st.selectbox(
        "Minority protection in acquisition",
        ["Off", "Maintain class ratio", "Minimum minority quota", "Minority-priority acquisition"],
        index=1,
    )

    initial_seed_strategy = st.selectbox(
        "Initial seed set strategy",
        ["Random", "Stratified", "Diverse seed"],
        index=1,
    )

    initial_seed_percent = st.select_slider(
        "Initial seed size",
        options=[5, 10, 15, 20],
        value=10,
    )
    ...
    if selection_strategy == "Hybrid: uncertainty + diversity":
        hybrid_weight = st.slider(
            ...
            0.5,
            0.05,
            help="0 = pure diversity, 1 = pure uncertainty",
        )
    else:
        hybrid_weight = 0.5
    ...
    evaluation_metric = st.selectbox(
        "Evaluation metric",
        ["Macro F1", "Weighted F1", "Minority recall", "Balanced accuracy"],
        index=0,
    )
    ...
    output_mode = st.selectbox(
        "Output mode",
        ["Learning curve", "Final metric table", "Both"],
        index=2,
    )
...
if run_q2:
    ...
    for c in metric_cols:
        agg_rows[f"{c}_mean"] = final_df[c].mean()
        agg_rows[f"{c}_std"] = final_df[c].std(ddof=0)

    best_metric_key = metric_key_from_ui(evaluation_metric)
    best_mean = agg_rows[f"{best_metric_key}_mean"]
    ...
    if not curve_df.empty:
        curve_agg = (
            curve_df.groupby("acquired_size", as_index=False)["metric_value"]
            .agg(["mean", "std"])
            .reset_index()
            .rename(columns={"mean": "metric_mean", "std": "metric_std"})
        )
    else:
        curve_agg = pd.DataFrame()

    res_summary = pd.DataFrame([agg_rows])

    st.session_state["q2_results"] = final_df
    st.session_state["q2_curve"] = curve_df
    st.session_state["q2_curve_agg"] = curve_agg
    st.session_state["q2_summary"] = res_summary
    ...
    st.session_state["q2_payload"] = {
        ...
        "settings": {
            ...
            "minority_protection": minority_protection,
            "initial_seed_strategy": initial_seed_strategy,
            "initial_seed_fraction": initial_seed_fraction,
            "distance_metric": distance_metric,
            "hybrid_weight": hybrid_weight,
    ...
    log_run(
        ...
        {
            "optimized_metric_mean": float(best_mean),
...
if output_mode in {"Final metric table", "Both"}:
...
if output_mode in {"Learning curve", "Both"} and curve_agg is not None and not curve_agg.empty:
    ...
    fig_curve = px.line(
        ...
        y="metric_mean",
        markers=True,
        title=f"Performance vs acquired subset size ({payload.get('best_metric_key', 'metric')})",
    )
...
best_metric_key = payload.get("best_metric_key", "macro_f1")
best_metric_mean = payload.get("best_metric_mean", None)
if best_metric_mean is not None:
    st.success(
        f"Best-performing current setup: {payload['settings']['selection_strategy']} "
        f"with mean {best_metric_key} = {best_metric_mean:.4f} over {payload['settings']['repeat_runs']} run(s)."
    )

st.download_button(
    "Download Q2 final metrics CSV",
    data=final_df.to_csv(index=False).encode("utf-8"),
    file_name="q2_final_metrics.csv",
    mime="text/csv",
)

if curve_agg is not None and not curve_agg.empty:
    st.download_button(
        "Download Q2 learning curve CSV",
        data=curve_agg.to_csv(index=False).encode("utf-8"),
        file_name="q2_learning_curve.csv",
        mime="text/csv",
    )
