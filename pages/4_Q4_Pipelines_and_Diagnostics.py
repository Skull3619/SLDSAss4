...
with st.sidebar:
    ...
    max_pipelines = st.slider(
        ...
        1,
        max(1, len(filtered_catalog)),
        min(20, max(1, len(filtered_catalog))),
        1,
    )
...
st.write(f"Pipelines that will actually run with current setting: **{min(max_pipelines, len(filtered_catalog))}**")
st.dataframe(filtered_catalog.head(60), use_container_width=True)
...
if res is None:
    st.info("Choose the Q4 settings in the sidebar and click **Run benchmark**.")
    st.stop()
...
if st.button("Run diagnostics for selected pipeline"):
...
if detail is None:
    st.info("Select a pipeline and click **Run diagnostics for selected pipeline**.")
    st.stop()
