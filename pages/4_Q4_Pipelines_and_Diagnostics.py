...
if run_q4:
    res = benchmark_pipelines(bundle.df, bundle.numeric_cols, bundle.target_col, specs=filtered_specs, test_size=test_size, oversample_train=oversample, random_state=int(random_state))
...
if st.button("Run selected pipeline diagnostics", use_container_width=True):
    ...
    detail = fit_pipeline_with_diagnostics(bundle.df, bundle.numeric_cols, bundle.target_col, spec_lookup[best_name], test_size=test_size, oversample_train=oversample, random_state=int(random_state))
